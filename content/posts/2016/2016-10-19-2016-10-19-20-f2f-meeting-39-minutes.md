...
Public Key Pinning dynamic list bug:

Windows store support bug:

Documentation on Windows store support:

HTTP password fields warning bug:

Intermediate CA crt.sh report:

CA Communication call for Qs:
...
Communication is still the same – via . Working to improve response time.
...
Slide 14 – Standards are available for download at . The most important standard for this group is EN 319 411. July 1, 2016 is the date when eIDAS comes into force.
...
## Day 2 – Thursday, October 20 {#Day_2\_-\_Thursday.2C_October_20}
...
### Network and Certificate System Security Requirements: Challenges, Interpretations {#Network_and_Certificate_System_Security_Requirements:\_Challenges.2C_Interpretations}
...
### Disclosure of Subordinate CAs {#Disclosure_of_Subordinate_CAs}
...
### Potential Change to Browser UI for Subject DN {#Potential_Change_to_Browser_UI_for_Subject_DN}
...
### Non-FIPS algorithms for customer public keys and certificate signing {#Non-FIPS_algorithms_for_customer_public_keys_and_certificate_signing}
